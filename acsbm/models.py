...
#     def simple(cls, beta: float, L: np.ndarray):
#         return Covariate(beta_matrix=beta * np.array(L))
    
#     def mixed(cls, beta: float, L: int, ):
#         return Covariate(beta_matrix = beta * Xm)
...
class NodeDataDistribution:
    ...
    def draw2(self, n: int, X: np.ndarray, random_state: np.random.Generator = None) -> np.ndarray:
        if random_state is None:
            random_state = np.random.default_rng()
