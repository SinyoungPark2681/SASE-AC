...
def initial_cluster(net: generation.GeneratedNetwork, k: int, d: int, random_state=0) -> NetworkInitialClusterResult:
    """
    ...
    for z in range(n_z):
        clusters = GaussianMixture(n_components=k, random_state=random_state, n_init=10).fit(X[S[z],:])
...
def initial_cluster_DA(net: generation.GeneratedNetwork, k: int, d: int, random_state=0) -> NetworkInitialClusterResult:
    """
    ACSBM Clustering Part 1:
    Recover theta_tilde (up to a permutation)
...
def initial_cluster2(model: models2.MultiCovariateModel, ndd: models2.NodeDataDistribution, n: int, A:np.ndarray, X:np.ndarray, k: int, d: int, random_state: np.random.RandomState) -> NetworkInitialClusterResult:
    """
    ACSBM Clustering Part 1:
    Recover theta_tilde (up to a permutation)
...
def reconcile_clusters2(model: models2.MultiCovariateModel, ndd: models2.NodeDataDistribution, n: int, A:np.ndarray, X:np.ndarray, initial_cluster: NetworkInitialClusterResult, random_state: np.random.RandomState) -> NetworkClusterResult:
    """
    ACSBM Clustering Parts 2 and 3:    
    Given an initial clustering (which separately clusters over the partitioned network),
    reconcile clusterings into a single set of clusters
    ...
    # estimate B_tilde, X_B
    A = A.astype(np.float64)
    B_tilde = count_connections(A, theta_tilde, dtype=float)
    B_tilde /= np.maximum(count_dyads(theta_tilde), 1)
    X_B = scaled_embeddings(B_tilde)
